...
The follow-up pass also transcribed the 2:14:45 Christmas bonus episode and two brief notices. Airtable subsequently supplied text for the short entries, which the library can prefer while retaining both source records. The musical parody *Nightmare On My Shelf* is particularly prone to speech-recognition errors; its generated text is a search aid, not verified lyrics.

The reusable runner is `scripts/transcribe_rss_audio.py`. It takes explicit publisher source keys, downloads their RSS enclosures, reuses a local model, and records completed results atomically. For example: `python scripts/transcribe_rss_audio.py sc-2233221167 --device cuda`. Install `faster-whisper` in an isolated environment first; GPU use additionally requires compatible NVIDIA libraries. CPU mode is available with `--device cpu`. Existing successful source records are skipped. Audio, model caches, and partial progress files are not published by the library builder.
